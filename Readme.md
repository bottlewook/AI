# 프론트엔드 개발 과제
## 사용법
git clone https://github.com/bottlewook/AI .
...
- [x] 이 과제에서는 `create-react-app`의 `react-scripts` 대신 직접 Webpack 설정을 구현해야 합니다. 초기 `index.js` 파일과 `webpack.config.js` 파일을 설정하여 React 애플리케이션을 빌드할 수 있도록 합니다.
...
- [x] 사용자가 여러 아이템을 선택하고, 이를 다른 칼럼으로 함께 드래그하여 이동할 수 있어야 합니다. <- shift를 이용하며 멀티 셀렉터 구현
